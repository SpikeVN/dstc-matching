...
export default function SwipeCard({ profile, style, className = '', showMoreButton = true, showSocials = true }) {
  ...
  const genderShown = profile.gender && profile.gender !== 'Không muốn nói';
        ...
        <div className="p-4 flex flex-col gap-3 flex-1">
          ...
          {skills.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {skills.map(skill => (
                <span
                  key={skill}
                  className={`text-[10px] px-2.5 py-0.5 rounded-full border font-body font-medium ${ENTRY_COLORS.skills}`}
                >
                  {skill}
                </span>
              ))}
          ...
          {goals.length > 0 && (
            <div className="flex items-center gap-2">
              <Target className="w-3 h-3 text-yellow-400/60 flex-shrink-0" />
              <div className="flex gap-1.5 flex-wrap">
                {goals.map(g => (
                  <span key={g} className={`text-[10px] px-2.5 py-0.5 rounded-full border font-body font-medium ${ENTRY_COLORS.goals}`}>
                    {g}
                  </span>
                ))}
              </div>
          ...
          {profile.achievements && (
            <div className="flex items-start gap-2">
              <Award className="w-3 h-3 text-red-400 flex-shrink-0 mt-0.5" />
              <p className="text-[10px] text-red-300/80 line-clamp-2 leading-relaxed">{profile.achievements}</p>
            </div>
          )}
        ...
        {showMoreButton ? (
          <>
            ...
            <button
              ...
              className="flex items-center justify-center gap-1 w-full text-[11px] text-muted-foreground/60 hover:text-primary/80 transition-colors py-2"
            >
              <span>Xem thêm</span>
            ...
            >
              ...
              {profile.bio && (
                <div>
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed whitespace-pre-line">{profile.bio}</p>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {profile.technical_skills.map(s => (
                      <span key={s} className={`text-xs px-3 py-1 rounded-full border font-body font-medium ${ENTRY_COLORS.skills}`}>{s}</span>
                    ))}
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {profile.soft_skills.map(s => (
                      <span key={s} className={`text-xs px-3 py-1 rounded-full border font-body font-medium ${ENTRY_COLORS.softSkills}`}>{s}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {goals.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {goals.map(g => (
                      <span key={g} className={`text-xs px-3 py-1 rounded-full border font-body font-medium ${ENTRY_COLORS.goals}`}>{g}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {profile.experience && (
                <div>
                  ...
                  <p className="text-xs text-purple-300/70">{profile.experience}</p>
                </div>
              )}
              ...
              {profile.achievements && (
                <div>
                  ...
                  <p className="text-xs text-red-300/70 leading-relaxed">{profile.achievements}</p>
                </div>
              )}
              ...
              {profile.achievements_other && (
                <div>
                  ...
                  <p className="text-xs text-red-300/70 leading-relaxed">{profile.achievements_other}</p>
                </div>
              )}
