    ...
    <aside className="w-64 h-screen flex flex-col sticky top-0 glass-card border-r border-neon/10">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 font-mono text-sm group ${
              isActive ?
      ...
      <div className="p-3 border-t border-neon/10 space-y-3">
        ...
        {currentUser?.role === 'admin' && (
          <Link
            to="/admin"
            className={`flex items-center gap-3 px-4 py-2 rounded-lg transition-all font-mono text-xs mb-1 ${
              location.pathname === '/admin' ? 'bg-neon/10 text-neon border border-neon/30' : 'text-muted-foreground hover:bg-neon/5 hover:text-neon/80 border border-transparent'
            }`}
          >
