...
function ChatBubble({ msg, isMe, senderProfile }) {
  return (
    <div className={`flex gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
      {!isMe && (
        <div className="w-7 h-7 rounded-lg overflow-hidden border border-primary/15 bg-muted/50 flex-shrink-0 mt-1">
      ...
      <div className={`max-w-[70%] ${isMe ? 'items-end' : 'items-start'} flex flex-col gap-0.5`}>
        ...
        <div className={`px-4 py-2.5 rounded-2xl text-sm font-body leading-relaxed ${isMe
          ? 'bg-primary/15 border border-primary/30 rounded-tr-sm text-foreground'
...
function ChatArea({ match, currentUser, profileMap, onBack }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Input */}
      <div className="p-3 pb-safe border-t border-primary/10 bg-background/60 backdrop-blur-md md:pb-3" style={{ paddingBottom: 'max(12px, env(safe-area-inset-bottom))' }}>
        <div className="flex gap-2 items-center">
          <Input
            ...
            className="font-body text-sm bg-black/30 border-primary/15 focus:border-primary/50 text-foreground placeholder:text-muted-foreground rounded-xl h-10"
          />
          <Button
            onClick={handleSend}
...
export default function Messages() {
  ...
  return (
    <div className="flex" style={{ height: '100dvh' }}>
      {/* Sidebar */}
      <div className={`w-full lg:w-72 flex-col border-r border-primary/10 bg-background/20 ${selectedMatch ? 'hidden lg:flex' : 'flex'}`}>
