...
# Use bash variable interpolation — psql -v variable substitution is unreliable
# with stdin heredocs in the Supabase postgres image.
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    EXCEPTION WHEN undefined_object THEN
      EXECUTE format('CREATE ROLE postgres LOGIN PASSWORD %L', '${POSTGRES_PASSWORD}');
    END \$\$;
    ...
    -- Grant CREATE on schemas for auth and rest migrations
    GRANT CREATE ON SCHEMA public TO supabase_auth_admin;
    GRANT CREATE ON SCHEMA public TO authenticator;
    GRANT ALL ON SCHEMA auth TO supabase_auth_admin;
    GRANT ALL ON SCHEMA storage TO supabase_auth_admin;
    GRANT ALL ON SCHEMA realtime TO supabase_auth_admin;

    -- Grant all on public tables to authenticated, service_role, and postgres (backend user)
...
# This requires superuser, so run as supabase_admin separately.
psql --username supabase_admin --dbname "$POSTGRES_DB" -c \
